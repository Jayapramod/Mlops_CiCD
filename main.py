...
class AgriculturalAssistant:
    ...
    def predict_crop_prices(self, crop_name, historical_data_path):
        ...
        try:
            ...
            try:
                ...
                chart_data = self.visualize_price_prediction(
    ...
    def visualize_price_prediction(self, actual_prices, predicted_prices, crop_name=None):
        """Return chart data for Chart.js visualization instead of matplotlib"""
        # Calculate confidence interval
        std_dev = float(np.std(actual_prices))
        
        # Prepare data for Chart.js
        chart_data = {
            'crop_name': crop_name if crop_name else 'All Crops',
            'historical_prices': [float(p) for p in actual_prices],
            'predicted_prices': [float(p) for p in predicted_prices],
            'confidence_upper': [float(p + std_dev) for p in predicted_prices],
            'confidence_lower': [float(p - std_dev) for p in predicted_prices],
            'stats': {
                'min_historical': float(np.min(actual_prices)),
                'max_historical': float(np.max(actual_prices)),
                'avg_historical': float(np.mean(actual_prices)),
                'min_predicted': float(np.min(predicted_prices)),
                'max_predicted': float(np.max(predicted_prices)),
